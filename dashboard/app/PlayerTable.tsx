      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-100 text-xs uppercase tracking-wider">
            ...
            <Th label="G+A"    k="contributions"     title="Goals + Assists" />
        ...
        <tbody>
          {sorted.map((p, i) => (
            <tr key={p.player} className={`${i !== sorted.length - 1 ? 'border-b border-gray-100' : ''} hover:bg-gray-50`}>
              ...
              <td className={`px-3 py-3 text-center tabular-nums font-bold ${sortKey === 'contributions' ? 'text-[#009C3B]' : 'text-gray-600'}`}>{p.contributions}</td>
